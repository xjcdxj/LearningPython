...
class Download(object):
    ...
    def _print(self):
        while True:
            ...
            if self.complete_ratio_info1:
                ...
                # print('\r%s  running %d threads' % (doc, self.theads_num), end='')
                print(self.complete_ratio_info1)
                print(num)
            time.sleep(1)

    def get_html(self, url, refer=None):
        rq = request.Request(url)
        rq.add_header('cookie', '__cfduid=dcdbb8756489b90242bc37d7f58684dfe1550717555')
        ...
        while count < 4:
            try:
                ...
                if html == 0:
                    count += 1
                    print('\033[1;31m \n%s failed, retry %d\033[0m!' % (url, count))
                    continue
                return html
            except:
                count += 1
                print('\033[1;31m \n%s timeout, retry %d\033[0m!' % (url, count))
        print('%s failed' % url)
        return None
    ...
    def parse_html_str(self, html_str, type):
        ...
        if type == 0:
            each_novel_urls = elements.xpath("//span[@class='s2']/a[@target='_blank']/@href")
            return each_novel_urls
        elif type == 1:
            each_chapter_temp = elements.xpath("//dd/a/@href")
            each_chapter = ['http://www.xiaoqiangxs.org{}'.format(i) for i in each_chapter_temp]
            del each_chapter_temp
            return each_chapter
        elif type == 2:
            chapter_title = elements.xpath("//div[@class='bookname']/h1/text()")[0]
    ...
    def save(self, novel_url, refer):
        dir = 'D://小说/xiaoqiang'
        novel_html_str = self.get_html(novel_url, refer).decode('gbk', 'ignore')
        novel_name = self.parse_html_str(novel_html_str, 3)[0]
        novel_name = novel_name.replace('?' and '|' and '\\' and '*' and '"' and ':' and '<' and '>' and '/', '')
        ...
        if novel_name + '.txt' not in exisited:
            ...
            for each_chapter in chapter_url:
                chapter_html_str = self.get_html(each_chapter, novel_url).decode('gbk', 'ignore')
                if chapter_html_str == 0:
                    print('%s failed' % each_chapter)
                    count += 1
                    continue
                ...
                with open(dir + '/' + novel_name + '.txt.temp', 'a', encoding='utf-8') as f:
                    ...
                    for each_line in chapter_txt:
                        f.write(each_line + '\n')
                    self.complete_ratio_info1[novel_name] = '%d/%d' % ( count, num)
                # print('\r%s %d/%d' % (novel_name, count, num), end='')
            self.theads_num -= 1
            os.rename(dir + '/' + novel_name + '.txt.temp', dir + '/' + novel_name + '.txt')
    ...
    def thread_run(self):
        ...
        try:
            ...
            pass
        url = ['http://www.xiaoqiangxs.org/wanben/1_{}'.format(i) for i in range(1, 37)]
...
def test():
    download = Download()
    download.save('http://www.xiaoqiangxs.org/1_1073/', 'http://www.xiaoqiangxs.org/wanben/1_1')
